...
class SeminarExceptionHandler {
    ...
    fun handler(e: MethodArgumentNotValidException): ResponseEntity<ErrorResponseList> {
        val request : HttpServletRequest =
            (RequestContextHolder.getRequestAttributes() as ServletRequestAttributes?)!!.request
        
        var errorList: List<ErrorReason> = 
            e.bindingResult.allErrors.map {
                ErrorReason(
                    field= (it as FieldError).field,
                    value= (it as? FieldError)?.rejectedValue,
    ...
    data class ErrorResponseList(
        val status: Int,
        val errors: List<ErrorReason>,
        val path: String,
    )
}